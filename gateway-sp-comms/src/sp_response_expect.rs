...
use crate::single_sp::HostBootfailPayloadData;
use crate::single_sp::HostPanicPayloadData;
use gateway_messages::ComponentActionResponse;
use gateway_messages::DiscoverResponse;
use gateway_messages::HF_PAGE_SIZE;
use gateway_messages::HostBootfailPayload;
use gateway_messages::HostPanicPayload;
...
expect_fn!(PowerStateWithReason(power_state_with_reason) -> PowerStateWithReason);
...
pub(crate) fn expect_component_vpd(
    r: (SocketAddrV6, SpResponse, Vec<u8>),
) -> Result<Vpd> {
    let peer = r.0;
    expect_data_fn!(ComponentVpd);
    let ((), data) = expect_component_vpd(r)?;
    let (vpd, remainder) = gateway_messages::deserialize(&data)
        .map_err(|err| CommunicationError::Deserialize { peer, err })?;
    if remainder.is_empty() {
        Ok(vpd)
    } else {
        Err(CommunicationError::UnexpectedTrailingData(remainder.to_vec()))
    }
}
...
pub(crate) fn expect_host_panic_payload(
    r: (SocketAddrV6, SpResponse, Vec<u8>),
) -> Result<HostPanicPayloadData> {
    // Wrapper around the autogenerated function for a nicer return type
    expect_data_fn!(HostPanicPayload(payload) -> HostPanicPayload);
    let (response, data) = expect_host_panic_payload(r)?;
    Ok(HostPanicPayloadData {
        total_len: response.total_len as usize,
        seqno: response.seqno,
        slot: response.slot,
        restart_id: response.restart_id,
        contents: data.to_vec(),
    })
}

pub(crate) fn expect_host_bootfail_payload(
    r: (SocketAddrV6, SpResponse, Vec<u8>),
) -> Result<HostBootfailPayloadData> {
    // Wrapper around the autogenerated function for a nicer return type
    expect_data_fn!(HostBootfailPayload(payload) -> HostBootfailPayload);
...
mod tests {
    ...
    fn test_component_vpd() {
        ...
        let len = gateway_messages::serialize(&mut data, &expected).unwrap();
        data.truncate(len);

        let actual = expect_component_vpd((
            dummy_addr(),
        ...
        let mut extra_data = data;
        extra_data.push(0xff);
        assert!(matches!(
            expect_component_vpd((
                dummy_addr(),
                SpResponse::ComponentVpd,
                extra_data,
            )),
        ...
        assert!(matches!(
            ...
            Err(CommunicationError::Deserialize { .. })
        ));
        ...
        assert!(matches!(
            ...
            Err(CommunicationError::BadResponseType {
                expected: "component_vpd",
                got: "component_action_ack",
            })
        ));
    }
